...
- **API** – `GET /api/v1/permission-coverage-reports` lists tenant/brand-scoped RBAC coverage snapshots, while `POST /api/v1/permission-coverage-reports` regenerates a snapshot for a supported module (`api`, `admin`, or `portal`). Individual records are available via `GET/PATCH/DELETE /api/v1/permission-coverage-reports/{id}`. Permissions: `security.permission_coverage.view` for read-only access, `security.permission_coverage.manage` for create/update/delete. Responses include coverage percentage, guarded versus unguarded route counts, hashed digests for any uncovered URIs, and the request correlation ID under `meta.correlation_id`. Validation and authorization failures return the shared `{ "error": { "code", "message" } }` schema with correlation metadata.
- **Automated analysis** – The `RoutePermissionCoverageAnalyzer` inspects registered Laravel routes for the `api`, `portal`, and Filament admin surfaces. Middleware such as `ability:*`, `permission:*`, `auth*`, and Filament guards count as RBAC protection; any uncovered route is captured as a hashed digest. Tests assert zero unguarded routes across the three modules so CI fails immediately if a new endpoint ships without permission middleware.
- **Policy & logging** – The `PermissionCoverageReportService` sanitises metadata, enforces tenant/brand scope, and emits structured JSON logs (correlation ID, module, execution time, uncovered count) alongside audit log entries on every create/update/delete. Spatie role assignments now provision `security.permission_coverage.view` for all tenant roles and `security.permission_coverage.manage` for administrators so RBAC checks remain deterministic.
- **Filament UI** – `/admin/permission-coverage-reports` provides tenant/brand scoped CRUD with module badges, coverage percent badges, and an indicator showing whether the latest snapshot meets the zero-unguarded requirement. Create/edit operations route through the same service layer to guarantee audit logging, while delete actions hash correlation IDs automatically. Metadata is explicitly marked NON-PRODUCTION for CI-only context.
- **Demo data** – `DemoDataSeeder` seeds a NON-PRODUCTION API module snapshot so environments immediately showcase the reporting dashboard. Factories are available for tests and local tinkering.

## RBAC Enforcement Gap Analyses

- **API** – `GET /api/v1/rbac-gap-analyses` returns tenant/brand scoped RBAC audit records with optional `status`, `brand_id`, `owner_team`, and `reference_id` filters. Administrators create, update, and delete entries via `POST/PATCH/DELETE /api/v1/rbac-gap-analyses/{id}`. Permissions: `security.rbac_gaps.view` grants read-only access for agents and viewers, while `security.rbac_gaps.manage` empowers administrators to maintain analyses. Every response emits the standard `{ "data": { ... } }` envelope and surfaces the correlation ID via response headers and `meta.correlation_id` for create/update flows. Validation and authorization errors continue to use `{ "error": { "code", "message" } }` payloads.
- **Service & logging** – The `RbacEnforcementGapAnalysisService` sanitises audit matrices, findings, and remediation plans, defaulting tenant/brand scope from the active request. Create/update/delete flows log JSON events (`rbac_gap_analysis.*`) with hashed meeting-minute digests, tenant/brand identifiers, and execution timings. Audit entries capture sanitized snapshots (priority, hashed summaries, remediation plan keys) so investigations maintain a durable trail without leaking PII.
- **RBAC provisioning** – `RolesAndPermissionsSeeder` and `TenantRoleProvisioner` now register `security.rbac_gaps.view` for admin/agent/viewer roles and `security.rbac_gaps.manage` for admins. Policies enforce tenant isolation and brand scope while respecting super-admin shortcuts via `Gate::before`.
- **Filament UI** – `/admin/rbac-gap-analyses` exposes CRUD screens with status badges, brand filters, and repeaters for audit matrix and findings. All destructive actions funnel through the service layer to ensure structured logging and audit trails. Helper copy flags remediation plans and notes as NON-PRODUCTION guidance.
- **Demo data & factories** – `DemoDataSeeder` seeds a NON-PRODUCTION RBAC gap analysis illustrating sample audit matrices and remediation milestones. A dedicated factory supplies repeatable fixtures for tests and local prototyping.
...
## SMTP Outbound Dispatcher

- **API** – `GET /api/v1/smtp-outbound-messages` lists queued email dispatches for the active tenant/brand while supporting optional `status`, `brand_id`, `ticket_id`, and `correlation_id` filters. `POST /api/v1/smtp-outbound-messages` queues a new message with HTML/text bodies, headers, CC/BCC/reply-to lists, and attachment metadata. `GET/PATCH/DELETE /api/v1/smtp-outbound-messages/{id}` manage individual dispatches before delivery. Permissions: `email.dispatches.view` grants read-only access for agents/viewers, while `email.dispatches.manage` allows queueing, editing, and deleting records.
- **Queue & retries** – `DispatchSmtpMessageJob` delivers messages on the dedicated `email` queue with exponential backoff (`60/180/360` seconds) and three attempts. The shared `SmtpOutboundMessageService` increments attempt counters, records structured logs with correlation IDs, and distinguishes transient exceptions from `SmtpPermanentFailureException` so permanent failures halt gracefully while transient errors are retried.
- **Logging & audits** – `SmtpOutboundMessageAuditLogger` stores hashed recipient/attachment digests in `audit_logs` and emits JSON logs (`smtp_outbound.*`) containing tenant, brand, ticket, status, and duration fields. All PII—addresses, headers, attachment paths—is hashed before logging to satisfy redaction requirements.
- **Filament UI** – `/admin/smtp-outbound-messages` provides tenant/brand scoped CRUD with repeater controls for recipients, attachment metadata, and key/value headers. Status badges highlight queued, retrying, sent, and failed states, while filters enable brand or status-specific triage.
- **Demo data** – `DemoDataSeeder` provisions a NON-PRODUCTION queued dispatch linked to the demo ticket so environments immediately showcase the workflow. Seeds flag everything as demo-only in headers and attachment names.
...
## Horizon Dashboard Infrastructure

- **Laravel Horizon configuration** – `config/horizon.php` ships with TLS-aware supervisor defaults, guard-aware middleware (`auth`, tenant resolution, two-factor enforcement, and RBAC via `infrastructure.horizon.view`), and Redis wait thresholds. Administrators can override supervisor counts using environment variables (`HORIZON_MAX_PROCESSES`, `HORIZON_QUEUE`, etc.) per environment.
- **API** – `GET /api/v1/horizon-deployments` (requires `infrastructure.horizon.view`) lists tenant/brand scoped Horizon deployments with optional `status`, `brand_id`, and `uses_tls` filters. `POST /api/v1/horizon-deployments`, `GET/PATCH/DELETE /api/v1/horizon-deployments/{id}` (requires `infrastructure.horizon.manage`) manage supervisor definitions, TLS metadata, and auth guard selection. Health probes live at `GET /api/v1/horizon-deployments/health` and `GET /api/v1/horizon-deployments/{id}/health`, returning structured summaries and correlation IDs using the standard `{ "error": { "code", "message" } }` failure envelope.
- **Global health integration** – `GET /api/v1/health` now includes a `horizon` block alongside the existing `db`/`redis` checks. The health controller executes supervisor validations via the new `HorizonDeploymentHealthService`, updates deployment records, and logs structured JSON metrics with hashed domains and correlation IDs.
- **Artisan tooling** – `php artisan horizon:health-check` iterates every deployment, emits structured logs, and returns non-zero exit codes when any supervisor fails or degrades. The scheduler invokes both `horizon:health-check` and `horizon:snapshot` every five minutes so CI/monitoring pipelines receive continuous coverage. Run `php artisan horizon` to launch the worker dashboard locally.
- **Filament UI** – `/admin/horizon-deployments` exposes tenant/brand scoped CRUD with queue tags, supervisor repeaters, TLS toggles, and metadata key/value fields. Status badges surface the most recent health check, and NON-PRODUCTION helper text mirrors the API. Demo data seeds a TLS-enabled deployment for exploration.
- **Demo + observability** – `DemoDataSeeder` provisions a NON-PRODUCTION Horizon deployment (domain `horizon.demo.localhost`, `sync` connection) so local environments can inspect the Filament resource and API responses immediately. All create/update/delete/health operations write audit log entries, emit JSON logs with SHA-256 domain digests, and respect per-tenant RBAC.

## Brand Configuration & Domains

- **API** – `GET /api/v1/brands` (requires `brands.view`) lists tenant-scoped brands and returns hashed domain digests alongside theme metadata. Administrators with `brands.manage` can create/update/delete brands through `POST /api/v1/brands`, `GET/PATCH/DELETE /api/v1/brands/{brand}`. Custom domains are managed via `GET /api/v1/brand-domains`, `POST /api/v1/brand-domains`, `GET/PATCH/DELETE /api/v1/brand-domains/{brandDomain}` (permissions: `brand_domains.view` / `brand_domains.manage`). All payloads follow the standard `{ "data": { ... } }` envelope with `{ "error": { "code", "message" } }` failures.
- **Domain verification** – `POST /api/v1/brand-domains/{brandDomain}/verify` queues DNS + SSL verification with structured JSON logs that redact hostnames using SHA-256 digests. The probe honours configurable CNAME targets (`config/branding.php`) and records verification, SSL issuer, correlation ID, and execution timings. Failures capture sanitized error codes while preserving audit trails.
- **Filament UI** – `/admin/brands` exposes scoped CRUD with logo uploads (stored on the configured `BRAND_ASSET_DISK`), live theme previews, and tenant filters. The domains relation manager adds inline creation, retryable verification actions, and status badges (pending, verifying, verified, failed) without leaving the brand record.
- **Brand assets & theme delivery** – `GET /api/v1/brand-assets` (permission: `brand_assets.view`) lists tenant-scoped logo, favicon, and stylesheet assets with hashed path digests and pagination. Administrators with `brand_assets.manage` can `POST /api/v1/brand-assets`, `GET/PATCH/DELETE /api/v1/brand-assets/{brandAsset}` to version assets. `GET /api/v1/brand-assets/{brandAsset}/deliver` returns CDN-aware URLs with `Cache-Control`, `ETag`, and correlation-aware JSON logs. Portals resolve color tokens and cache-busted asset URLs via `GET /api/v1/brands/{brand}/theme`, which falls back to defaults defined in `config/branding.php` when tenant assets are missing. Filament operators manage the catalog at `/admin/brand-assets` with tenant/brand filters and NON-PRODUCTION helper text.
- **Observability & audit** – Every create/update/delete/verify operation writes an `audit_logs` entry plus structured JSON logs containing correlation IDs, tenant/brand IDs, hashed domains, and duration metrics. Jobs retry according to `BRAND_VERIFICATION_MAX_ATTEMPTS` and reset domain status automatically on failure.
- **Demo data** – `DemoDataSeeder` provisions a NON-PRODUCTION `support.demo.localhost` domain so local environments demonstrate the domain lifecycle out of the box. Demo assets live under `storage/app/public/brands/*` and are safe placeholders.
